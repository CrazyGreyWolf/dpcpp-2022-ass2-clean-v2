...
std::ostream& operator<<(std::ostream& os, const Patient& p)
{
  ...
  os << "|";
  for (int i = 0; i < (int)p.vitals().size(); ++i) {
    auto v = p.vitals()[i];
    os << *v;
...
void Patient::addVitals(const Vitals* v)
{
  ...
  AlertLevelContext context;

  // Calculate the alarm level
  if (primaryDiagnosis() == Diagnosis::BONEITIS) {

    context.set(std::make_shared<AlertLevelBoneitisStrategy>());
  }
  else if (primaryDiagnosis() == Diagnosis::GREYSCALE) {

    context.set(std::make_shared<AlertLevelGreyscaleStrategy>());
  }
  else if (primaryDiagnosis() == Diagnosis::SIMIAN_FLU) {

    context.set(std::make_shared<AlertLevelSimianFluStrategy>());
  }

  context.calculate(this, v);
}
